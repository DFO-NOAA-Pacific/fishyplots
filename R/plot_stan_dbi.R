...
#' @param regions center or subregions to be plotted. Takes a character list of region names OR a single center name ( "NWFSC", "AFSC", or "PBS").
...
plot_stan_dbi <- function(species, regions) {
 ...
 data(all.dbi)
  ...
  for (center in unique(all.dbi$science_center)) {
    ...
    filtered <- stand_data %>%
      dplyr::filter(common_name == species|scientific_name == species, region %in% regions)
  ...
  plot <- ggplot2::ggplot(data = combined_df, 
    ...
    ggplot2::geom_point() +
    ggplot2::geom_ribbon(ggplot2::aes(x = year, ymin = stand_lwr, ymax = stand_upr, fill = region), color = NA, alpha = 0.2) +
